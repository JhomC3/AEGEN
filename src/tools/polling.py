    ...
    try:
        ...
        if "api.telegram.org" in url:
            ...
            if "Network is unreachable" in str(e.reason):
                print(
                    "CONSEJO: Tu instancia no tiene salida a Telegram. Prueba configurando TELEGRAM_PROXY en tu .env"
                )
        return None
...
def forward_update(update):
    ...
    try:
        ...
        if result:
            ...
            print(
                f"❌ ERROR: Local API at {API_URL} failed to process Update {update_id}."
            )
            print("   Check if the bot container is running and port 8000 is exposed.")
    except Exception as e:
...
def main():
    ...
    while True:
        try:
            ...
            if updates is None:
                print(
                    "⚠️ Warning: No response from Telegram. Possible network issue or rate limit."
                )
                time.sleep(5)
                continue
