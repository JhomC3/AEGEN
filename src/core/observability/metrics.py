...
class LLMCallMetrics(BaseModel):
    ...
    end_time: Optional[float] = Field(None, description="Timestamp fin")
    latency_ms: Optional[float] = Field(None, description="Latencia en ms")
    
    # Tokens
    input_tokens: Optional[int] = Field(None, description="Tokens entrada")
    output_tokens: Optional[int] = Field(None, description="Tokens salida")
    total_tokens: Optional[int] = Field(None, description="Total tokens")
    
    # Result
    success: bool = Field(True, description="Llamada exitosa")
    error_message: Optional[str] = Field(None, description="Mensaje error")
    
    # Cost
    estimated_cost_usd: Optional[float] = Field(None, description="Costo estimado USD")
    ...
    def _set_end_time(self, end_time: Optional[float]) -> None:
