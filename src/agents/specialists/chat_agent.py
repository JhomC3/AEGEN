...
try:
    ...
    ResourceExhaustedError: Any = google_exceptions.ResourceExhausted
    GoogleAPICallError: Any = google_exceptions.GoogleAPICallError
except ImportError:
...
def _load_persona_prompt() -> str:
    ...
    try:
        ...
        return (
            """Eres un asistente útil. Usuario: {user_name}. Mensaje: {user_message}."""
        )
...
async def _enhanced_conversational_response(
    ...
    if (not conversation_history or conversation_history == "") and memory_data.get(
        "buffer"
    ):
        buffer_text = "\n".join([
    ...
    try:
        ...
        logger.error(
            f"Error crítico en _enhanced_conversational_response: {e}", exc_info=True
        )
...
async def _enhanced_chat_node(state: GraphStateV2) -> dict[str, Any]:
    ...
    if file_id:
        ...
        "archivo" in user_message.lower()
        or "documento" in user_message.lower()
        or "pdf" in user_message.lower()
    ):
