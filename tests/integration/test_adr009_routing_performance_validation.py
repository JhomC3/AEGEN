...
class TestADR009RoutingPerformance:
    ...
    def test_function_calling_implementation(self):
        ...
        # Check file exists
        assert os.path.exists(routing_tools_path), f"Routing tools file missing: {routing_tools_path}"
        
        with open(routing_tools_path, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        assert len(missing_content) == 0, f"Missing function calling content: {missing_content}"
    ...
    def test_routing_analyzer_migration(self):
        ...
        if os.path.exists(routing_analyzer_path):
            with open(routing_analyzer_path, 'r', encoding='utf-8') as f:
                content = f.read()
            ...
            assert len(found_indicators) >= 1, f"No function calling patterns found in routing analyzer"
    ...
    def test_adr009_documentation_exists(self):
        ...
        assert len(missing_adr_content) <= 1, f"ADR-0009 missing content: {missing_adr_content}"
    ...
    def test_performance_improvement_calculation(self):
        ...
        assert improvement_factor >= 15, f"Performance improvement should be 15x+, got {improvement_factor:.1f}x"
        assert new_performance <= 2.0, f"New performance should be ≤2s, got {new_performance}s"
