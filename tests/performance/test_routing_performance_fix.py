...
class TestRoutingPerformanceFix:
    ...
    def test_function_calling_tool_creation(self, mock_langchain):
        ...
        expected_tool_params = [
            'intent', 'confidence', 'target_specialist', 
            'entities', 'requires_tools', 'subintent', 'next_actions'
        ]
    ...
    def test_routing_analyzer_initialization(self, mock_langchain, mock_routing_components):
        """
    ...
    async def test_function_calling_response_time(self, mock_routing_components):
        ...
        assert elapsed_time < 2.0, f"Function calling took {elapsed_time:.2f}s, target is <2s"
    ...
    def test_chat_agent_restoration_structure(self, mock_langchain):
        ...
        print(f"✅ ChatAgent restored: {actual_line_count} lines (target: {expected_line_count}+)")
    ...
    async def test_end_to_end_performance_simulation(self, mock_routing_components):
        ...
        chat_response = "¡Hola! Muy bien, gracias por preguntar. ¿En qué puedo ayudarte hoy?"
    ...
    def test_memory_performance_restoration(self, mock_routing_components):
        ...
        assert active_features == 5, f"Expected 5 memory features, got {active_features}"
        
        # Simulate memory access time (should be fast)
        memory_access_time = 0.02  # 20ms for memory operations
        assert memory_access_time < 0.1, f"Memory access too slow: {memory_access_time}s"
...
class TestPerformanceRegression:
    ...
    def test_no_structured_output_usage(self, mock_langchain):
        ...
        assert len(structured_output_usage) == 0, f"Structured output still used: {structured_output_usage}"
