        ...
        for i, item in enumerate(knowledge_data, 1):
            try:
                ...
                if result.get("success", False):
                    ...
                    print(
                        f"❌ {i}/{len(knowledge_data)} - Error: {result.get('error', 'Unknown')}"
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Cargar conocimiento CBT a la global knowledge base"
    )
    parser.add_argument("--file", type=str, help="Archivo JSON con conocimiento CBT")
    parser.add_argument(
        "--create-sample", action="store_true", help="Crear archivo de ejemplo"
    )
